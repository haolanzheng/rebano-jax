...
def train_pinn(state, col_points, quad_weights, loss_weights, alpha,
               max_steps=10000, tol_grad=1e-8, wandb_config=None):
    """Train the PINN model on the Poisson problem."""
    
    def residual_loss_pinn(apply_fn, params, batch_data, quad_weights, loss_data):
        return poisson_residual_loss(apply_fn, params, batch_data, quad_weights, loss_data)
    
    def boundary_loss_pinn(apply_fn, params, batch_data, quad_weights, loss_data=None):
        return poisson_boundary_loss(apply_fn, params, batch_data, quad_weights)

    loss_fns = {'residual': residual_loss_pinn,
                'boundary': boundary_loss_pinn}
    ...
    for step in pbar:
        state, loss_weights, metrics = step_fn(state, loss_weights,
                                               col_points, quad_weights,alpha=alpha)
        
        pbar.set_postfix({
            'Total': f"{metrics['total_loss']:.2e}",
            'Resid': f"{metrics['residual_loss']:.2e}",
            'BC': f"{metrics['boundary_loss']:.2e}"
        })
        ...
        if step % 1000 == 0:
            tqdm.write(f"Step {step}: Total={metrics['total_loss']:.5e}, "
                      f"Resid={metrics['residual_loss']:.5e}, "
                      f"BC={metrics['boundary_loss']:.5e}")
...
def train_rebano(config_rebano, pinns, col_points, quad_weights,
    ...
    return train_rebano_pmap(config_rebano, pinns, col_points, 
                             quad_weights, f_data, u_xx_precomp, u_bc_precomp, n_devices, batch_size, available_devices, wandb_config)
...
def train_rebano_pmap(config_rebano, pinns, col_points, quad_weights,
    ...
    def train_single_sample(f_sample):
        ...
        # Get final loss
        _, _, final_metrics = step_fn(final_state, quad_weights, final_loss_weights, adaptive_weights=update_weights, alpha=alpha)
    ...
    for batch_idx in pbar_batches:
        ...
        try:
            batch_losses = pmap_train_batch(batch_f_data)
...
def main():
    ...
    if config.wandb.enabled:
        try:
            wandb.init(
                ...
                name=f"poisson_rebano_neurons_{config.num_neurons}",
    ...
    inputs  = np.load(config.data.inputs_dir)[::config.data.sub_x, offset:n_train+offset].astype(np.float32)
    outputs = np.load(config.data.outputs_dir)[::config.data.sub_x, offset:n_train+offset].astype(np.float32)  
    ...
    x_resid, quadw_resid = spatial_grid1d(Xi, Xf, Nx, sampling_method=config.domain.sampling_method)
    x_bc, quadw_bc = spatial_grid1d_bc(Xi, Xf)
    x_test, _ = spatial_grid1d(Xi, Xf, Nx, sampling_method='uniform')
    ...
    weights_pinn = {'residual': w_resid_pinn,
                    'boundary': w_bc_pinn}
    
    alpha_pinn       = train_pinn_config.alpha
    ...
    dummy_key     = jax.random.PRNGKey(0)
    sample_params = pinn_sample.init(dummy_key, col_points['residual'])
    ...
    for i, key in enumerate(keys[num_pretrain_neurons:]):
        ...
        f_pinn = inputs[:, idx_max:idx_max+1]
        
        print(f"\nStart training PINN #{n+1} with input function index {idx_max} ...\n")
        
        loss_data = {'residual': {'f': f_pinn},
                     'boundary': None}
        ...
        pinn_state = train_pinn(pinn_state, col_points, quad_weights,
                                weights_pinn, alpha_pinn, 
        ...
        metadata = {
            'training resolution': f_pinn.shape[0],
            'input function': f_pinn,
            'l2_error': float(l2_error),
            'pinn_config': config.pinn.to_dict()
        }
    
        save_pinn_checkpoint(pinn_state, config.pinn.train.save_dir + f"poisson_pinn_{n+1}", metadata)
        ...
        u_xx = compute_lap_u_scalar(apply_fn, params, x_resid).reshape(-1, 1)
        u_xx_precomp = u_xx_precomp.at[n].set(u_xx)
        u_bc = vmap(lambda x: apply_fn(params, x))(x_bc).reshape(-1, 1)
        u_bc_precomp = u_bc_precomp.at[n].set(u_bc)
        ...
        idx_max, loss_max = train_rebano(train_rebano_config, 
                                         pinn_list, col_points, quad_weights, inputs, u_xx_precomp[:n+1], u_bc_precomp[:n+1], available_devices, use_pmap, batch_size, wandb_config=config.wandb)
